...
+ function($) {
    ...
    var Picker = function (params) {
        ...
        p.cols = [];
        p.initialized = false;
        p.dateType = params.dateType; // xiaominzh add
        ...
        p.layout = function () {
            var pickerHTML = '';
            var pickerClass = '';
            var dateTypeClass = ""; // xiaominzh add
            if(p.dateType=="day"){
                dateTypeClass="date-type-day";
            }else if(p.dateType=="week"){
                dateTypeClass="date-type-week";
            }else if(p.dateType=="month"){
                dateTypeClass="date-type-month";
            }else if(p.dateType=="quarter"){
                dateTypeClass="date-type-quarter";
            }else if(p.dateType=="year"){
                dateTypeClass="date-type-year";
            }
            var i;
            p.cols = [];
            var colsHTML = '';
            ...
            pickerHTML =
                ...
                '<div class="picker-modal-inner picker-items '+dateTypeClass+'">' +
                colsHTML +
...
+ function($) {
    ...
    // 创建指定日期的时间
    function createDate(currentDate,offsetDate){
        var date = new Date(currentDate);
        var curDate = date.getDate()+offsetDate;
    ...
    var initWeeks = (function(){
        ...
        var dateTmp = new Date(startDate);

        var weeks = [];

        while(dateTmp.getTime()<endDate.getTime()){
            //todo: week start at monday
            var offset = dateTmp.getDay()*(-1)+1;
            var weekStart = createDate(dateTmp,offset);
            var weekEnd = createDate(dateTmp,6+offset);

            weeks.push(simpleDateFormat(weekStart)+"至"+simpleDateFormat(weekEnd));
            dateTmp = createDate(dateTmp,7);
        }
        return weeks;
    })();

    var getWeek = function(date){
        var offset = date.getDay()*(-1)+1;
        var weekStart = createDate(date,offset);
        var weekEnd = createDate(date,6+offset);
        return simpleDateFormat(weekStart)+"至"+simpleDateFormat(weekEnd);
    };
    ...
    var initQuarters = (function(){
        ...
        for(var year=2015;year<=new Date().getFullYear();year++){
            quarters.push(year+"一季度");
            quarters.push(year+"二季度");
            quarters.push(year+"三季度");
            quarters.push(year+"四季度");
        }

        return quarters;
    })();

    var getQuarter = function(year,month){
        if(month<3){
            return year+"一季度";
        }else if(month<6){
            return year+"二季度";
    ...
    var dateTypeQuarter = {
        ...
        value: [getQuarter(today.getFullYear(),today.getMonth())],
    ...
    $.fn.datetimePicker = function(params) {
        return this.each(function() {
            ...
            if(params.dateType=="day"){
                p = $.extend(p, dateTypeDay);
            }else if(params.dateType=="week"){
                p = $.extend(p, dateTypeWeek);
            }else if(params.dateType=="month"){
                p = $.extend(p, dateTypeMonth);
            }else if(params.dateType=="quarter"){
                p = $.extend(p, dateTypeQuarter);
            }else if(params.dateType=="year"){
                p = $.extend(p, dateTypeYear);
            }
            p = $.extend(p, params);
            ...
            if (params.value){
                ...
                if(params.dateType=="day"){
                    $(this).val(value[0]+"-"+value[1]+"-"+value[2]);
                }else if(params.dateType=="week"){
                    $(this).val(value[0]);
                }else if(params.dateType=="month"){
...
+function($) {
     ...
     */
    Router.prototype.load = function(url, ignoreCache,isPushState) {
        if (ignoreCache === undefined) {
            ignoreCache = false;
        }

        if (this._isTheSameDocument(location.href, url)) {
            this._switchToSection(Util.getUrlFragment(url),isPushState);
        } else {
            this._saveDocumentIntoCache($(document), location.href);
            this._switchToDocument(url, ignoreCache,isPushState);
        }
    };
     ...
     * @private
     */
    Router.prototype._switchToSection = function(sectionId,isPushState) {
        if (!sectionId) {
            return;
        }
        ...
        if(!!isPushState){
            ...
            this._replaceState('#' + sectionId, sectionId);
        }
    ...
    Router.prototype._doSwitchDocument = function(url, isPushState, direction) {
        ...
        if (isPushState) {
            ...
            this._replaceState(url, $visibleSection.attr('id'));
        }
    };
    ...
    Router.prototype._onPopState = function(event) {
        ...
        if (state.id < lastState.id) {
            var that = this;
            require(["page_source_mapping","web_utils"],function(pageSourceMapping,WebUtils){
                var pathname = state.url.full;
                var pageName = WebUtils.getPageName(pathname);
                var deps = pageSourceMapping[pageName]||[];
                require(deps,function(){
                    that._back(state, lastState);
                });
            });
        } else {
    ...
    Router.prototype._replaceState = function(url, sectionId) {
        var state = {
            id: this._getNextStateId(),
            pageId: sectionId,
            url: Util.toUrlObject(url)
